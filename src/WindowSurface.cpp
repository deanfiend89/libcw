...
WindowSurface::WindowSurface() {
	...
	mSurfaceComposerClient = surfaceComposerClient;
    mSurfaceControl = sc;
}
...
sp<Surface> WindowSurface::getSPSurface() const {
    return mSurfaceControl->getSurface();
}

void WindowSurface::setSize(int width, int height){
	mSurfaceComposerClient->openGlobalTransaction();
	mSurfaceControl->setSize(width, height);
	mSurfaceComposerClient->closeGlobalTransaction();
}

void WindowSurface::setPosition(int x, int y){
	mSurfaceComposerClient->openGlobalTransaction();
	mSurfaceControl->setPosition(x, y);
	mSurfaceComposerClient->closeGlobalTransaction();	
}

void WindowSurface::setLayer(int layer){
	mSurfaceComposerClient->openGlobalTransaction();
	mSurfaceControl->setLayer(layer);
	mSurfaceComposerClient->closeGlobalTransaction();	
}

int WindowSurface::getDisplayWidth(){
    status_t err;
    // Get main display parameters.
    sp<IBinder> mainDpy = SurfaceComposerClient::getBuiltInDisplay(
            ISurfaceComposer::eDisplayIdMain);
    ...
    if (mainDpyInfo.orientation != DISPLAY_ORIENTATION_0 &&
            mainDpyInfo.orientation != DISPLAY_ORIENTATION_180) {
        // rotated
        return mainDpyInfo.h;
    } else {
        return mainDpyInfo.w;
    }
}

int WindowSurface::getDisplayHeight(){
    status_t err;
    // Get main display parameters.
    sp<IBinder> mainDpy = SurfaceComposerClient::getBuiltInDisplay(
            ISurfaceComposer::eDisplayIdMain);
